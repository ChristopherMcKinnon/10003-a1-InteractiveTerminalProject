...
// Scene 2.1 - Call the intruder to come out (BAD)==========================================================================================================
CallIntruderOut:;
...
if (playerInput == 1) // Kick open the front door (BAD)
{
    badDecisions += 1;
    goto BadDecisionsHandler;
}
...
if (playerInput == 1) // Enter the bedroom (BAD)
{
    badDecisions += 1;
    goto BadDecisionsHandler;
}
...
else if (playerInput == 2) // Check under the bed
{
    ...
    Console.WriteLine("The room is cold. A thick set of blinds covers the window. Moon and stars hang from the ceiling. A babies room. Shifting your flashlight to the half-open closet door, shadows seem to slink around the beam of light. Is it the headache?\n(1) Check the closet\n(2) Check under the bed\n(3) Look inside the crib\n(4) Go back");
    goto Bedroom;
}
...
{
    Console.WriteLine($"The wind begins to howl outside as you leave the room. There isn't much time left.\n{currentBadDecisionDialogue}\nPress enter to continue:");
    Console.ReadLine();
    goto MainFloor;
}
